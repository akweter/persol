  ...
  <?php
    ...
    if(isset($_POST['delete_checkout_product'])){
      ...
      // $Erase = mysqli_query($PDO, "DELETE FROM `cart` WHERE c_Sku = $new_SKU");
      // unlink('../../../public/img/'.$Erase['image']);

      // $Erase = mysqli_query($PDO, "DELETE FROM `cart` WHERE cid = $delete_product_id");
      // $Erase = mysqli_query($PDO, "DELETE FROM `wishlist` WHERE wid = $delete_product_id"); 950
...
<html lang="en">
    ...
    <body>
      ...
      <div class="container">
        <main>
          <?php if (isset($promo_notice)) { echo($promo_notice); } if (isset($cart_null_message)) { echo($cart_null_message); } if (isset($user_exist)) { echo($user_exist); } if (isset($order_received)) { echo($order_received); }?>
            ...
            <div class="col-md-5 col-lg-4 order-md-last">
              <h4 class="d-flex justify-content-between align-items-center mb-3">
                ...
                <li class="list-group-item d-flex justify-content-between">
                  <span><h3>Total (GHS)</h3></span>
                  <strong><h3>¢<?php if(isset($discount)){echo($disount_given);}else{echo($cart_price_value);} ?></h3></strong>
                </li>
              </ul>
            </div>
            <div class="col-md-7 col-lg-8">
              <h4 class="mb-3">Billing address</h4>
              <form method="post" action="" name="final_checkout_form">
                <div class="row g-3">
                  <div class="col-sm-6">
                  ...
                  <div class="col-md-4">
                    ...
                    <select value="<?php if (isset($user_region)){echo($user_region);}?>" name="P_region" class="form-select" id="region" required>
                ...
                <div class="my-3">
                  <div class="form-check">
                    <input id="creditiCard" name="paymentMode" type="radio" class="form-check-input" checked required>
                    <label class="form-check-label" data-bs-toggle="dropdown" for="creditiCard">Credit card</label>
                    <!-- <div class="dropdown" id="dropdown">
                      <ul class="dropdown-menu">
                        <div class="row gy-3">
                    ...
                    <div class="form-check">
                      <input id="mobileMoney" name="paymentMode" type="radio" class="form-check-input" required>
                      <label class="form-check-label" for="mobileMoney">Mobile Money</label>
                    </div>
                    <div class="form-check">
                      <input id="payOnDelivery" name="paymentMode" type="radio" class="form-check-input" required>
                      <label class="form-check-label" for="payOnDelivery">Pay On Delivery</label>
                    </div>
                    <div class="form-check">
                      <input id="bank" name="paymentMode" type="radio" class="form-check-input" required>
                      <label class="form-check-label" for="bank">Bank Payment</label>
                    </div>
                  </div>

                <hr class="my-4">
                <button class="w-100 btn btn-outline-primary btn-light btn-lg" type="submit">Continue to checkout</button>
                <hr class="my-4 mb-5">
              </form>
            </div>
          </div>
